...
a+=h)])}break;default:t.console.log("unknown message:",f.toString(16))}break;default:throw Error("invalid status");}k=a-p;p=b.subarray(p,p+k);p[0]=f;h instanceof y&&"NoteOn"===h.f&&0===h.a&&(h.f=B[8],p=new Uint8Array([128|h.b,h.F,h.a]));r.push(p);x.push(h)}c.m.push(x);c.G.push(r)};function M(){this.l=5E5;this.j=!1;this.b=0;this.H=this.I=this.D=this.C=!1;this.u=1;this.J=16383}q=M.prototype;q.R=function(c){this.C=c};q.S=function(c){this.D=c};q.U=function(c){this.I=c};q.T=function(c){this.H=c};q.A=function(){var c;this.i=!0;this.g=Date.now();if(this.a)for(c=0;16>c;++c)this.a.postMessage("midi,b"+c.toString(16)+",78,0")};q.N=function(){return this.a};
function N(c){c.A();O(c);c.i=!1;c.h=null;c.g=-1;c.K=null;c.s=null;c.B=null;clearTimeout(c.c);c.j?P(c):c.a.addEventListener("message",function(d){"link,ready"===d.data&&P(c)},!1)}function O(c){c.l=5E5;c.b=0}q.P=function(){var c=this;if(!this.a)throw Error("WebMidiLink not found");this.j?(this.h instanceof Array&&this.b>=this.h.length&&(this.b=0),Q(this)):this.a.addEventListener("message",function(d){"link,ready"===d.data&&(c.j=!0,Q(c))},!1)};
...
function Q(c){function d(){var c=a[k].time,n=a.length,h,m,p=Date.now();if(e.i)e.g=Date.now()-e.g;else{do{h=a[k].event;"SetTempo"===h.f&&(e.l=h.data[0]);"ControlChange"===h.f&&111===h.F&&(g[0]={pos:k});if("Marker"===h.f&&("A"===h.data[0]&&(g[0]={pos:k}),"B"===h.data[0]&&e.D&&g[0]&&"number"===typeof g[0].pos)){k=g[0].pos;e.c=setTimeout(d,0);e.b=k;return}if("Marker"===h.f&&(h=h.data[0].match(/^LOOP_(START|END)=ID:(\d+),COUNT:(-?\d+)$/)))if("START"===h[1])g[h[2]|0]=g[h[2]]||{pos:k,count:h[3]|0};else if("END"===
h[1]&&e.I){m=g[h[2]|0];if(0!==m.count){0<m.count&&m.count--;k=m.pos;e.c=setTimeout(d,0);e.b=k;return}g[h[2]|0]=null}f.postMessage(a[k++].webMidiLink)}while(k<n&&a[k].time===c);k<n?(p=Date.now()-p,e.c=setTimeout(d,e.l/(1E3*b)*(a[k].time-c-p)*(1/e.u))):e.C&&g[0]&&"number"===typeof g[0].pos?(k=g[0].pos,e.c=setTimeout(d,0)):e.H&&(O(e),Q(e));e.b=k}}var e=c,b=c.K.Y,a=c.h,f=c.a,k=c.b||0,g=[];c.i?(c.c=setTimeout(d,c.g),c.i=!1,c.g=-1):c.c=setTimeout(d,c.l/1E3*b*c.h[0].time)}
